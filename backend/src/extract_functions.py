...
class AgdaExtractor:
    ...
    def classify_tokens(
        self, tokens: List[str], declared: Dict[str, str], synthetic_vars: set
    ) -> Tuple[set, set, set]:
    ...
    def shallow_trace(self, sig: str, vars_: set, ops: set, nums: set) -> str:
        ...
        def annotate_segment(segment: str) -> str:
            ...
            return re.sub(
                pattern,
                lambda m: self.annotate_token_for_trace(m.group(), vars_, ops, nums),
                segment,
            )

        return ", ".join(annotate_segment(part) for part in parts)

    def annotate_token_for_trace(
        self, token: str, vars_: set, ops: set, nums: set
    ) -> str:
        if token in vars_:
            return f"var {token}"
        elif token in ops:
...
class DatabaseClient:
    ...
    def insert_into_db(self, rows):
        ...
        INSERT INTO agda_signatures (
            ...
            annotated_signature, shallow_trace, line_no
        )
        VALUES %s
